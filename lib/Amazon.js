...
class AmazonScraper {
    ...
    extractProductVariants($, body) {
        ...
        try {
            ...
            const json = /jQuery\.parseJSON\('(\{[^']+\})'\);/.exec(body);
            if (json) {
    ...
    grabAsinDetails(body) {
        ...
        if (body.indexOf('aplus_feature_div') > -1 && !output.description) {
            output.description = $('#aplus_feature_div').html();
            if(output.description && output.description.trim().length >= 1) {
                output.description = striptags(output.description.trim().replace(/<(script|style)[^>]*?>.*?<\/\1>/gis, ''));
            }
        }

        if (body.indexOf('plus') > -1 && !output.description) {
            output.description = $('#plus').html();
            if(output.description && output.description.trim().length >= 1) {
                output.description = striptags(output.description.trim().replace(/<(script|style)[^>]*?>.*?<\/\1>/gis, ''));
            }
        }

        if (body.indexOf('aplus') > -1 && !output.description) {
            output.description = $('#aplus').html();
            try {
                output.description = removeAllTags(output.description);
            } catch {
                // continue regardless of error
            }
        }
    ...
    removeAllTags(string, remove_breaks = false) {
        // Remove content within <script> and <style> tags
        string = string.replace(/<(script|style)[^>]*?>.*?<\/\1>/gis, '');
      
        // Strip all remaining HTML tags
        string = striptags(string);
      
        // Remove line breaks and extra whitespace if requested
        if (remove_breaks) {
    ...
    extractImages($, body) {
        ...
        const imgRegex =  /'colorImages': \{ 'initial': (.+)\}/.exec(body);
        if (imgRegex) {
            try {
